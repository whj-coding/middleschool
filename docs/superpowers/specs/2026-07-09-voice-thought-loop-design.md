...
练习页仍展示默认答案、步骤和语音说思路区域。语音区域通过 placeholder 展示静态示例，实际转写状态默认为空，避免把示例误记为学生证据。
...
- 失败时保留学生已输入的内容，并显示“语音转写暂不可用，可继续手动输入思路。”。
...
提交时 `recordInteraction()` 仍使用 `action: "submit_answer"`。`studentAnswer` 始终包含答案和步骤；只有学生手动输入或成功获得 STT 转写时，才追加第三段语音证据：
